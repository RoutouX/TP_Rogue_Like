...
bool SquareHitBox2D::itHit(HitBox* pHitBox) {
    if (dynamic_cast<SquareHitBox2D *>(pHitBox) != nullptr) {
        ...
        long double circleDistanceX = abs(
                dynamic_cast<RoundHitBox2D *>(pHitBox)->getX() - this->getX() - this->getWidth() / 2);
        long double circleDistanceY = abs(
                dynamic_cast<RoundHitBox2D *>(pHitBox)->getY() - this->getY() - this->getHeight() / 2);
        if (circleDistanceX >
            (this->getWidth() / 2 + dynamic_cast<RoundHitBox2D *>(pHitBox)->getRayon())) { return false; }
        if (circleDistanceY >
            (this->getHeight() / 2 + dynamic_cast<RoundHitBox2D *>(pHitBox)->getRayon())) { return false; }
        if (circleDistanceX <= (this->getWidth() / 2)) { return true; }
        if (circleDistanceY <= (this->getHeight() / 2)) { return true; }
        double cornerDistance_sq =
                sqrt(circleDistanceX - this->getWidth() / 2) + sqrt(circleDistanceY - this->getHeight() / 2);
